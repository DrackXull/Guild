...
function rebuildCharacterStats(data) {
  if (!data.characters) {
    data.characters = [];
  }
  ...
  (data.reports || []).forEach((report) => {
    ...
    const character = characterMap.get(report.characterId);
    if (!character) return;
    const stats = report.stats || {};
    const confirmed = (report.screenshots || []).length > 0;
    ...
    character.stats.extractions += report.extracted ? 1 : 0;
    if (confirmed) {
...
function requireAdmin(data, playerId) {
  const player = findPlayer(data, playerId);
...
function createApplicant(payload) {
  return persist((data) => {
    ...
    const player = {
      id,
      discordTag: payload.discordTag,
      displayName: payload.displayName || payload.discordTag || payload.email || 'Applicant',
      rank: 'Applicant',
      role: 'guest',
      isMember: false,
      friends: [],
...
function listPlayers() {
  ...
  return data.players.map((player) => {
    ...
    const characterIds = new Set((player.characterIds || []).filter(Boolean));
    const characters = (data.characters || []).filter((char) => characterIds.has(char.id));
    const confirmedKills = characters.reduce((sum, char) => sum + (char.stats?.confirmedKills || 0), 0);
    const confirmedBossKills = characters.reduce((sum, char) => sum + (char.stats?.confirmedBossKills || 0), 0);
    const confirmedExtractions = characters.reduce((sum, char) => sum + (char.stats?.extractions || 0), 0);
    const verifiedRuns = (data.runs || []).filter((run) => run.isVerified && (run.participants || []).includes(player.id));
    const modeCounts = {};
    ...
    const modeBreakdown = Object.entries(modeCounts)
      .map(([mode, count]) => ({ mode, count, percent: verifiedRuns.length ? Math.round((count / verifiedRuns.length) * 100) : 0 }))
      .sort((a, b) => b.count - a.count);
    const teammateCounts = {};
    verifiedRuns.forEach((run) => {
      (run.participants || []).forEach((participantId) => {
        if (participantId === player.id) return;
        teammateCounts[participantId] = (teammateCounts[participantId] || 0) + 1;
      });
    });
    const topTeammates = Object.entries(teammateCounts)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 5)
      .map(([teammateId, count]) => {
        const teammate = data.players.find((p) => p.id === teammateId);
        return { id: teammateId, displayName: teammate?.displayName || 'Unknown', runsTogether: count };
      });
    const playerReports = (data.reports || []).filter((report) => report.playerId === player.id && runMap.get(report.runId)?.isVerified);
    const bossesByMode = {};
    playerReports.forEach((report) => {
      const run = runMap.get(report.runId);
      if (!run) return;
      const mode = run.mode || 'Unknown';
      const bossKills = report.stats?.bossKills || 0;
      bossesByMode[mode] = (bossesByMode[mode] || 0) + bossKills;
    });
    return {
      ...player,
      ...
      profileStats: {
        ...
        reportsSubmitted: playerReports.length,
        modeBreakdown,
        topTeammates,
        bossesByMode
      }
...
function submitReport(payload) {
  ...
  return persist((data) => {
    ...
    if (run.isVerified) {
      const firstReportHonor =
        settings.runVerification.firstReportHonor ?? settings.runVerification.reportHonor ?? settings.runVerification.reportGF ?? 0;
      const confirmingHonor =
        settings.runVerification.confirmingReportHonor ?? settings.runVerification.reportHonor ?? settings.runVerification.reportGF ?? 0;

      run.reporterIds.forEach((playerId, index) => {
        if (run.reportHonorGranted?.includes(playerId)) return;
        const honorAmount = index === 0 ? firstReportHonor : confirmingHonor;
        awardHonor(playerId, honorAmount, 'report_submission', { runId: run.id, reportId: id }, adminPlayerId);
        run.reportHonorGranted = Array.from(new Set([...(run.reportHonorGranted || []), playerId]));
      });
    }
...
function createApplication(payload) {
  return persist((data) => {
    ...
    if (!payload.applicantPlayerId) {
      throw new Error('Applicant profile is required before submitting.');
    }

    const applicant = findPlayer(data, payload.applicantPlayerId);
    if (applicant.isMember) {
      throw new Error('Members do not need to apply.');
    }
    ...
    const application = {
      ...
      applicantName: payload.applicantName || applicant.displayName,
      applicantPlayerId: payload.applicantPlayerId,
      discordTag: payload.discordTag,
      email: payload.email || '',
...
function listApplicationsForApplicant(playerId) {
  const data = loadAndHydrate();
  return (data.applications || [])
    .filter((application) => application.applicantPlayerId === playerId)
    .map((application) => ({
      id: application.id,
      code: application.code,
      status: application.status,
      isRepeat: application.isRepeat,
      repeatMatches: application.repeatMatches,
      reviewTrail: application.reviewTrail,
...
function upsertSettings(partial, adminPlayerId = null) {
  return persist((data) => {
    ...
    requireAdmin(data, adminPlayerId);
    data.settings = {
      ...data.settings,
      ...partial,
      drasticScore: {
