...
const DEFAULT_SETTINGS = {
  ...
  theme: { accent: '#d47a0b', accentStrong: '#f3a43b', background: '#050304', panel: '#0d0c0f' }
};
...
function ensureSettings(data) {
  ...
  data.settings.traitOptions = data.settings.traitOptions || DEFAULT_SETTINGS.traitOptions;
  data.settings.theme = {
...
function getPlayerLedgerTotals(data, playerId) {
  ...
  entries.forEach((entry) => {
    current += entry.changeAmount;
    if (entry.direction === 'earn') {
      lifetime += entry.changeAmount;
    }
    if (current > maxHonor) {
      maxHonor = current;
    }
  });
  return { current, lifetime, maxHonor };
}
...
function deriveRank(maxHonor = 0, settings = {}) {
  const bands = settings.rankBands || [];
  const sorted = bands.slice().sort((a, b) => (a.minMaxHonor || 0) - (b.minMaxHonor || 0));
  let picked = sorted[0] || { name: 'Initiate', minMaxHonor: 0 };
  sorted.forEach((band) => {
    if (maxHonor >= (band.minMaxHonor || 0)) {
      picked = band;
    }
  });
  return picked;
...
function createPlayer(payload, adminPlayerId = null) {
  return persist((data) => {
    const hasOfficer = (data.players || []).some((player) => player.role === 'officer' || player.role === 'admin');
    const isBootstrap = (data.players || []).length === 0 || !hasOfficer;
...
function listPlayers() {
  ...
  return data.players.map((player) => {
    ...
    const rankBand = deriveRank(ledger.maxHonor, data.settings);
    return {
      ...player,
      lifetimeHonor: ledger.lifetime,
      currentHonor: ledger.current,
      maxHonor: ledger.maxHonor,
      rankBand,
      presence
    };
  });
...
function createApplication(payload) {
  return persist((data) => {
    ...
    if (!payload.applicantName || !payload.discordTag) {
      throw new Error('Applicant name and Discord tag are required.');
    }
    ...
    const roleStyles = Array.isArray(payload.roles) ? payload.roles.filter(Boolean) : [];
    const classSelections = Array.isArray(payload.classes) ? payload.classes.filter(Boolean) : [];

    if (!roleStyles.length) {
      throw new Error('Select at least one preferred role style.');
    }
    if (!classSelections.length) {
      throw new Error('Select at least one main class.');
    }
    ...
    const application = {
      ...
      availability: {
        ...
        localWindow: formatLocalWindow(payload.availabilityLocal, payload.availabilityLocalEnd),
        easternWindow: convertWindowToEastern(
          formatLocalWindow(payload.availabilityLocal, payload.availabilityLocalEnd),
          payload.timeZone
        )
      },
      gameplay: {
        hoursInGame: Number(payload.hoursInGame || 0),
