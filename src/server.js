...
const server = http.createServer(async (req, res) => {
  ...
  const allowUnauthed =
    ...
    url.pathname === '/api/recruits/register' ||
    url.pathname === '/api/recruits/status' ||
    (url.pathname === '/api/applications' && method === 'POST');
  ...
  try {
    ...
    if (url.pathname === '/api/recruits/register' && method === 'POST') {
      const body = await readBody(req);
      const recruit = registerRecruit(body);
      send(res, 201, recruit);
      return;
    }

    if (url.pathname === '/api/recruits/status' && method === 'GET') {
      const playerId = url.searchParams.get('playerId');
      send(res, 200, listRecruitApplications(playerId));
      return;
    }
