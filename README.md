...
- Tracks the highest HP a player has ever held at once—even if they spend some later
- Drives suggested rank bands (Initiate → Apprentice → Wanderer → Pathfinder → Vanguard → Exemplar → Council) so officers can gate perks on sustained contributions
...
Theme palette (accent glow, highlight, background, panel tint)
...
- Applicants pick official servers (US East/West, EU Central, East Asia Seoul/Tokyo, SEA Singapore, Oceania Sydney, South America SaoPaulo), declare their time zone, and enter local play windows that auto-render in US Eastern for consistent scheduling.
...
Honor stats (current/max/lifetime), applications (with time zone + server fields), and roster data already flow through a thin persistence layer, so mapping `loadData`/`saveData` to Supabase or Firestore drivers is the only change needed to harden auth while keeping the Officer Lounge private.
...
| POST | `/api/recruits/register` | Create a guest/account-level profile before submitting an application |
| GET | `/api/recruits/status?playerId=...` | Let recruits see their own application status without member keys |
...
### Manual roster seeding (for tests/demo)

You can add a member and an officer quickly with curl while keys are still file-backed:

```bash
curl -X POST http://localhost:3000/api/players \
  -H 'Content-Type: application/json' \
  -H 'X-Officer-Key: guild-officer-demo-key' \
  -d '{"displayName":"Tester","discordTag":"tester#0001","rank":"Recruit","role":"member","isMember":true}'

curl -X POST http://localhost:3000/api/players \
  -H 'Content-Type: application/json' \
  -H 'X-Officer-Key: guild-officer-demo-key' \
  -d '{"displayName":"Officer","discordTag":"officer#0001","rank":"Officer","role":"officer","isMember":true}'

# Recruit-only profile + application
curl -X POST http://localhost:3000/api/recruits/register \
  -H 'Content-Type: application/json' \
  -d '{"displayName":"Prospect","discordTag":"prospect#9999"}'

# Use the returned `id` to submit an application that officers can score together
curl -X POST http://localhost:3000/api/applications \
  -H 'Content-Type: application/json' \
  -d '{"playerId":"<recruit-id>","applicantName":"Prospect","discordTag":"prospect#9999","server":"NA-East","hoursInGame":120,"favoriteMode":"High Roller","mostPlayedMode":"PvP","daysPerWeek":3,"usualDays":"Tue/Thu/Sat","timeWindow":"Evenings","roles":["Fighter"],"characters":[{"name":"TorchKnight"}],"bosses":"Lich, Ghost King","notes":"I play melee support"}'
```
