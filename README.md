...
### Login-first homepage and quick test accounts

- The root page is always the login overlay. Members sign in with the guild name, handle, and member key; officers add the officer key to unlock the lounge; applicants create an applicant profile from the same screen before filling out the Recruit form.
- Default demo keys live in settings (`guild-member-demo-key` / `guild-officer-demo-key`). To create seeded testers against a fresh data file, use the built-in admin (the first player is auto-promoted to admin on empty data) to provision more accounts:

```bash
ADMIN_ID=$(curl -s -H "X-Guild-Key:guild-member-demo-key" -H "X-Officer-Key:guild-officer-demo-key" http://localhost:3000/api/players | jq -r '.[0].id')
curl -s -X POST -H "Content-Type: application/json" -H "X-Guild-Key:guild-member-demo-key" -H "X-Officer-Key:guild-officer-demo-key" -d '{"displayName":"Officer Demo","discordTag":"officer#0001","role":"admin","isMember":true,"adminPlayerId":"'"$ADMIN_ID"'"}' http://localhost:3000/api/players
curl -s -X POST -H "Content-Type: application/json" -H "X-Guild-Key:guild-member-demo-key" -H "X-Officer-Key:guild-officer-demo-key" -d '{"displayName":"Member Demo","discordTag":"member#0001","role":"member","isMember":true,"adminPlayerId":"'"$ADMIN_ID"'"}' http://localhost:3000/api/players
```

- Applicants can self-register with `POST /api/applicants` (exposed via the login overlay) and submit applications that officers can review together; applicants can also check their own status through `/api/applications?applicantPlayerId=...` and the Profile tab.
