...
- 🔁 Real-time refresh of settings, players, characters, runs, reports, bounties, presence, and the admin log (manual refresh or auto every 45s).
...
- 🧭 Presence radar + sticky headbar showing who is online or in-game, with a quick-toggle popover of everyone currently active; feeds off `/api/presence/ping` and the optional desktop helper.
- ✉️ Messages dropdown surfaces run tags, LFG updates, and report mentions for the active actor; badges dim automatically when nothing is waiting.
