...
3. The homepage is always the login overlay. Guild members enter the guild name + member key (officers also enter the officer key). Recruits create an applicant profile from the same screen, then finish the Recruit application; their status shows up in the Profile section and via `/api/applications?applicantPlayerId=...`.
