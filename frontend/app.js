...
const elements = {
  ...
  profileModes: document.getElementById('profileModes'),
  profileTeammates: document.getElementById('profileTeammates'),
  profileRuns: document.getElementById('profileRuns'),
  profileApplications: document.getElementById('profileApplications')
};
...
function renderMetrics() {
  ...
  const guildStats = computeGuildStats();
  const metrics = [
    { label: 'Active now', value: onlineCount },
    { label: 'In game', value: inGameCount },
    { label: 'Members', value: memberCount },
    { label: 'Officers', value: officerCount },
    { label: 'Confirmed kills', value: guildStats.confirmedKills },
    { label: 'Bosses downed', value: guildStats.confirmedBossKills },
    { label: 'Verified runs', value: guildStats.verifiedRuns },
    { label: 'Open LFG posts', value: openLfg },
    { label: 'Characters', value: state.characters.length },
    { label: 'Runs logged', value: guildStats.totalRuns },
    { label: 'Reports', value: state.reports.length },
    { label: 'Bounties active', value: guildStats.activeBounties },
...
function computeGuildStats() {
  ...
  const extracts = state.characters.reduce((sum, character) => sum + (character.stats?.extractions || 0), 0);
  const verifiedRuns = state.runs.filter((run) => run.isVerified).length;
  const activeBounties = state.bounties.filter((bounty) => bounty.isActive !== false).length;
  return {
    confirmedKills,
    confirmedBossKills,
    extracts,
...
function renderProfile() {
  ...
  const actor = getActor();
  if (!actor) {
    elements.profileStats.innerHTML = '<div class="empty-state">Select a player to see their dossier.</div>';
    return;
  }
  const stats = actor.profileStats || {};
  elements.profileStats.innerHTML = `
    <article class="card">
      <p class="eyebrow">${escapeHTML(actor.displayName)}</p>
      <h4>Verified record</h4>
      <div class="grid two-cols">
        <div><p class="subtitle">Kills</p><strong>${stats.confirmedKills || 0}</strong></div>
        <div><p class="subtitle">Bosses</p><strong>${stats.confirmedBossKills || 0}</strong></div>
        <div><p class="subtitle">Extractions</p><strong>${stats.confirmedExtractions || 0}</strong></div>
        <div><p class="subtitle">Verified runs</p><strong>${stats.verifiedRuns || 0}</strong></div>
      </div>
      <p class="subtitle">${stats.reportsSubmitted || 0} reports submitted</p>
    </article>
  `;

  if (elements.profileModes) {
    elements.profileModes.innerHTML = stats.modeBreakdown?.length
      ? stats.modeBreakdown
          .map(
            (mode) => `<li><span>${escapeHTML(mode.mode)}</span><span>${mode.count} · ${mode.percent}%</span></li>`
          )
          .join('')
      : '<li>No verified runs yet.</li>';
  }

  if (elements.profileTeammates) {
    elements.profileTeammates.innerHTML = stats.topTeammates?.length
      ? stats.topTeammates
          .map((mate) => `<li><span>${escapeHTML(mate.displayName)}</span><span>${mate.runsTogether} runs</span></li>`)
          .join('')
      : '<li>No teammate history yet.</li>';
  }

  if (elements.profileRuns) {
    const recentRuns = state.runs
      .filter((run) => (run.participants || []).includes(actor.id) && run.isVerified)
      .sort((a, b) => new Date(b.scheduledAt || b.createdAt) - new Date(a.scheduledAt || a.createdAt))
      .slice(0, 5);
    elements.profileRuns.innerHTML = recentRuns.length
      ? recentRuns
          .map(
            (run) => `
              <article class="card">
                <h4>${escapeHTML(run.code || 'RUN')}</h4>
                <p class="subtitle">${escapeHTML(run.mode || '')} · ${formatDate(run.scheduledAt || run.createdAt)}</p>
                <p class="subtitle">Participants: ${(run.participants || [])
                  .map((id) => escapeHTML(state.players.find((p) => p.id === id)?.displayName || 'Unknown'))
                  .join(', ')}</p>
              </article>
            `
          )
          .join('')
      : '<div class="empty-state">Verified runs will appear here after two players sign off.</div>';
  }

  if (elements.profileApplications) {
    const myApplications = state.applications.filter((app) => app.applicantPlayerId === actor.id);
    elements.profileApplications.innerHTML = myApplications.length
      ? myApplications
          .map(
            (app) => `
              <article class="card">
                <h4>${escapeHTML(app.code || 'APP')}</h4>
                <p class="subtitle">Status: ${escapeHTML(app.status)}</p>
                <p class="subtitle">Submitted ${formatDate(app.createdAt)}</p>
                <p class="subtitle">Reviews: ${(app.reviewTrail || []).length}</p>
              </article>
            `
          )
          .join('')
      : '<div class="empty-state">Applications you submit will show here.</div>';
  }
}
...
function populateSettingsForm() {
  ...
  if (form.firstReportHonor) {
    form.firstReportHonor.value = runVerification.firstReportHonor ?? runVerification.reportHonor ?? 0;
  }
  if (form.confirmingReportHonor) {
    form.confirmingReportHonor.value = runVerification.confirmingReportHonor ?? runVerification.reportHonor ?? 0;
  }
  if (form.reportHonor) {
...
async function loadData() {
  ...
  try {
    ...
    const [settings, players, runs, reports, bounties, lfgPosts, characters] = await Promise.all([
      fetchJSON('/settings'),
      fetchJSON('/players'),
      fetchJSON('/runs'),
      fetchJSON('/reports'),
    ...
    state.applications = actorIsOfficer() && players.length
      ? await fetchJSON('/applications')
      : getActorId()
        ? await fetchJSON(`/applications?applicantPlayerId=${getActorId()}`)
        : [];
    state.adminLog = actorIsOfficer() ? await fetchJSON('/admin-log') : [];
...
function guardAdminAction() {
  ...
  if (!actor || actor.role !== 'admin') {
    showToast('Admins only—log in with an admin profile to change those settings.', 'error');
    setView(actorIsMember() ? 'member' : 'recruit');
    return false;
  }
  return true;
...
document.body.classList.add('view-recruit');
setupScrollLinks();
document.querySelectorAll('#viewToggle button').forEach((button) => {
...
const applicantForm = document.getElementById('applicantSignup');
if (applicantForm) {
  applicantForm.addEventListener('submit', async (event) => {
    event.preventDefault();
    const displayName = document.getElementById('applicantName')?.value.trim();
    const discordTag = document.getElementById('applicantDiscord')?.value.trim();
    if (!displayName || !discordTag) {
      showToast('Name and Discord are required to create an applicant profile.', 'error');
      return;
    }
    const payload = {
      displayName,
    ...
    try {
      const applicant = await fetchJSON('/applicants', { method: 'POST', body: JSON.stringify(payload) });
      setMemberKey(getMemberKey());
      setSession({ guildName: elements.loginGuildName?.value.trim() || 'Guild Nexus', username: applicant.displayName, actorId: applicant.id });
      showToast('Applicant profile created—finish the Recruit form to submit.', 'success');
      setView('recruit');
      loadData();
    } catch (error) {
      showToast(error.message, 'error');
...
if (elements.applicationScheduleForm) {
  elements.applicationScheduleForm.addEventListener('submit', async (event) => {
    ...
    if (!getActorId()) {
      showToast('Create an applicant profile on the login page first.', 'error');
      return;
    }
...
document.getElementById('settingsForm').addEventListener('submit', async (event) => {
  ...
  const payload = {
    ...
    runVerification: {
      ...
      firstReportHonor: Number(formData.get('firstReportHonor')),
      confirmingReportHonor: Number(formData.get('confirmingReportHonor')),
...
function bootstrap() {
  ...
  if (savedSession) {
    if (savedSession.memberKey) setMemberKey(savedSession.memberKey);
    if (savedSession.officerKey) setOfficerKey(savedSession.officerKey);
    if (elements.loginGuildName && savedSession.guildName) {
      elements.loginGuildName.value = savedSession.guildName;
    }
    if (elements.loginUsername && savedSession.username) {
      elements.loginUsername.value = savedSession.username;
    }
    if (savedSession.actorId) {
      activeActorId = savedSession.actorId;
    }
  }
