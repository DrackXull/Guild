const state = {
  ...
  adminLog: [],
  recruitStatus: [],
  session: { authed: false, username: '', guildName: '' },
  settings: {
...
const elements = {
  ...
  loginGuildName: document.getElementById('loginGuildName'),
...
function setSession(session) {
  state.session = { ...state.session, ...session, authed: true };
  ...
  const persistable = {
    guildName: state.session.guildName,
    username: state.session.username,
    memberKey: getMemberKey(),
    officerKey: getOfficerKey()
  };
  localStorage.setItem(SESSION_KEY, JSON.stringify(persistable));
  if (state.session.guildName) {
    document.title = `${state.session.guildName} | Guild Nexus Honor Deck`;
  }
}
...
if (elements.loginForm) {
  elements.loginForm.addEventListener('submit', (event) => {
    ...
    const guildName = elements.loginGuildName?.value.trim();
    const username = elements.loginUsername?.value.trim();
    const memberKey = elements.loginPassword?.value.trim();
    if (!guildName || !username || !memberKey) {
      showToast('Guild, username, and password are required.', 'error');
      return;
    }
...
function bootstrap() {
  ...
  if (savedSession?.memberKey) {
    setMemberKey(savedSession.memberKey);
    if (savedSession.officerKey) {
