const DEFAULT_GUILD_NAME = 'Guild Soon™';
const DEFAULT_THEME = {
  accent: '#d47a0b',
  accentStrong: '#f3a43b',
  background: '#050304',
...
const state = {
  ...
  recruitStatus: [],
  recruitProfile: null,
  session: { authed: false, username: '', guildName: DEFAULT_GUILD_NAME },
  settings: {
...
const elements = {
  ...
  loginAccessKey: document.getElementById('loginAccessKey'),
  openRecruitLink: document.getElementById('openRecruitLink'),
  ...
  guildTotals: document.getElementById('guildTotals'),
  ...
  recruitProfileForm: document.getElementById('recruitProfileForm'),
  ...
  profileActorSlot: document.getElementById('profileActorSlot')
};
...
const SESSION_KEY = 'guildSession';
const RECRUIT_PROFILE_KEY = 'guildRecruitProfileId';
const RECRUIT_PROFILE_DATA_KEY = 'guildRecruitProfileData';
...
let recruitProfileId = localStorage.getItem(RECRUIT_PROFILE_KEY) || '';
let profileTargetId = localStorage.getItem('guildProfileTarget') || '';
...
function applyTheme(theme = {}) {
  ...
  root.style.setProperty('--accent', next.accent || DEFAULT_THEME.accent);
  root.style.setProperty('--accent-strong', next.accentStrong || next.accent || DEFAULT_THEME.accentStrong);
  root.style.setProperty('--bg', next.background || DEFAULT_THEME.background);
  root.style.setProperty('--panel', next.panel || DEFAULT_THEME.panel);
}

function persistFormState(formId, storageKey) {
  ...
  if (saved) {
    try {
      ...
      console.warn('Unable to restore form state', storageKey, error);
    }
  }
...
function getProfileTargetId() {
  if (profileTargetId && state.players.some((player) => player.id === profileTargetId)) {
    return profileTargetId;
  }
  profileTargetId = getActorId();
  return profileTargetId;
}

function setProfileTargetId(value) {
  profileTargetId = value;
  renderProfile();
  localStorage.setItem('guildProfileTarget', profileTargetId);
}
...
function convertWindowToEastern(localWindow = '', timeZone = '') {
  if (!localWindow) return '';
  const [startRaw, endRaw] = localWindow.split('-').map((part) => String(part || '').trim());
  ...
  if (startMin === null || endMin === null) return localWindow;
  const offsetLocal = timezoneOffsetMinutes(timeZone);
  const offsetEastern = timezoneOffsetMinutes('America/New_York');
  const diff = offsetLocal - offsetEastern;
  const normalize = (minutes) => {
    const shifted = minutes - diff;
    const wrapped = ((shifted % 1440) + 1440) % 1440;
    const h = String(Math.floor(wrapped / 60)).padStart(2, '0');
    const m = String(wrapped % 60).padStart(2, '0');
    return `${h}:${m}`;
  };
  return `${normalize(startMin)} - ${normalize(endMin)} ET`;
}
...
function renderMetrics() {
  ...
  const totalHonor = state.players.reduce((sum, player) => sum + (player.currentHonor || 0), 0);
  const metrics = [
    ...
    { label: 'HP bank', value: totalHonor }
  ];
...
function renderGuildTotals() {
  if (!elements.guildTotals) return;
  const confirmedKills = state.characters.reduce((sum, character) => sum + (character.stats?.confirmedKills || 0), 0);
  const confirmedBossKills = state.characters.reduce((sum, character) => sum + (character.stats?.confirmedBossKills || 0), 0);
  const extracts = state.reports.filter((report) => report.extracted).length;
  elements.guildTotals.innerHTML = `
    <div class="total-pill">
      <p class="eyebrow">Confirmed kills</p>
      <strong>${confirmedKills}</strong>
      <span class="subtitle">Screenshot-backed tallies</span>
    </div>
    <div class="total-pill">
      <p class="eyebrow">Bosses felled</p>
      <strong>${confirmedBossKills}</strong>
      <span class="subtitle">Updated as officers confirm</span>
    </div>
    <div class="total-pill">
      <p class="eyebrow">Extractions</p>
      <strong>${extracts}</strong>
      <span class="subtitle">Across all recorded runs</span>
    </div>
  `;
}

function renderTopbarStats() {
  if (!elements.topbarStats) return;
  const confirmedKills = state.characters.reduce((sum, character) => sum + (character.stats?.confirmedKills || 0), 0);
  const confirmedBossKills = state.characters.reduce((sum, character) => sum + (character.stats?.confirmedBossKills || 0), 0);
  const verifiedRuns = state.runs.filter((run) => run.isVerified).length;
  const completedBounties = state.bounties.filter((bounty) => bounty.status === 'complete' || bounty.status === 'completed').length;
  elements.topbarStats.innerHTML = `
    <div class="topbar-chip">
      <span class="eyebrow">Confirmed kills</span>
      <strong>${confirmedKills}</strong>
    </div>
    <div class="topbar-chip">
      <span class="eyebrow">Bosses</span>
      <strong>${confirmedBossKills}</strong>
    </div>
    <div class="topbar-chip">
      <span class="eyebrow">Verified runs</span>
      <strong>${verifiedRuns}</strong>
    </div>
    <div class="topbar-chip">
      <span class="eyebrow">Bounties done</span>
      <strong>${completedBounties}</strong>
    </div>
  `;
}
...
function renderProfile() {
  ...
  const targetId = actorIsOfficer() ? getProfileTargetId() : getActorId();
  const player = state.players.find((p) => p.id === targetId);
  if (!player) {
    elements.profileSummary.innerHTML = '<div class="empty-state">Pick a player to view their dossier.</div>';
    return;
  }
  const playerCharacters = state.characters.filter((char) => char.playerId === player.id);
  const verifiedRuns = state.runs.filter((run) => run.isVerified && (run.participants || []).includes(player.id));
  const reportsByPlayer = state.reports.filter((report) => report.playerId === player.id && report.statsApplied);
  const totalKills = playerCharacters.reduce((sum, char) => sum + (char.stats?.confirmedKills || 0), 0);
  const totalBossKills = playerCharacters.reduce((sum, char) => sum + (char.stats?.confirmedBossKills || 0), 0);
  const extracts = reportsByPlayer.filter((report) => report.extracted).length;
  const perMode = verifiedRuns.reduce((map, run) => {
  ...
  verifiedRuns.forEach((run) => {
    (run.participants || []).forEach((id) => {
      if (id === player.id) return;
      teammateCounts[id] = (teammateCounts[id] || 0) + 1;
    });
  });
  const topTeammates = Object.entries(teammateCounts)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 3)
    .map(([id, count]) => {
      const mate = state.players.find((p) => p.id === id);
      return `${mate?.displayName || 'Unknown'} (${count})`;
    });
  ...
  elements.profileSummary.innerHTML = `
    ...
    <div class="metric-card">
      <span>${totalBossKills}</span>
      <p>Bosses felled</p>
    </div>
  `;

  const modeLines = Object.entries(perMode)
    .map(([mode, count]) => `<li>${escapeHTML(mode)} · ${count}</li>`)
    .join('');
  const mateLines = topTeammates.map((mate) => `<li>${escapeHTML(mate)}</li>`).join('');
  elements.profileBreakdown.innerHTML = `
    <h4>Habits & squadmates</h4>
    <p class="subtitle">Auto-updates after two players sign off a run.</p>
    <ul class="pill-list">${modeLines || '<li>No verified runs yet.</li>'}</ul>
    <h5>Who you queue with</h5>
    <ul class="pill-list">${mateLines || '<li>Run with guildmates to build history.</li>'}</ul>
  `;

  elements.profileRecent.innerHTML = recentReports.length
    ? recentReports
        .map((report) => {
          ...
          return `
            <article class="card search-result">
              <h4>${escapeHTML(report.code || 'Report')} · ${escapeHTML(character?.name || '')}</h4>
              <p class="subtitle">${escapeHTML(run?.mode || '')} · ${formatDate(report.createdAt)}</p>
              <p>${escapeHTML(report.comment || 'No notes')}</p>
            </article>
          `;
        })
        .join('')
    : '<div class="empty-state">Fill out runs and get a second signer to see your dossier.</div>';

  if (elements.profileActorSlot) {
    if (actorIsOfficer()) {
      const options = state.players
        .map((player) => `<option value="${player.id}" ${player.id === targetId ? 'selected' : ''}>${escapeHTML(player.displayName)}</option>`)
        .join('');
      elements.profileActorSlot.innerHTML = `<label class="inline">View as<select id="profileActorSelect">${options}</select></label>`;
      const select = document.getElementById('profileActorSelect');
      if (select) {
        select.addEventListener('change', (event) => setProfileTargetId(event.target.value));
      }
    } else {
      elements.profileActorSlot.innerHTML = `<p class="subtitle">Viewing ${escapeHTML(player.displayName)}</p>`;
    }
  }
}
    ...
    .map((player) => {
      ...
      return `
        <article class="card player-card">
          <div>
            <h4>${escapeHTML(player.displayName)} <span class="sub">${escapeHTML(player.rankBand?.name || player.rank)}</span></h4>
          ...
          <div class="honor-pill">
            <span>Max ${player.maxHonor || 0} HP</span>
            <span>Current ${player.currentHonor || 0} HP</span>
            <span>Lifetime ${player.lifetimeHonor || 0} HP</span>
          </div>
          ...
          <div>
            ...
            <div class="application-grid">
              <div>
                ...
                <p class="subtitle">${escapeHTML(application.availability?.localWindow || '')} local</p>
                <p class="subtitle">${escapeHTML(application.availability?.easternWindow || '')}</p>
                <p class="subtitle">Timezone: ${escapeHTML(application.availability?.timeZone || 'Not provided')}</p>
              </div>
              <div>
...
function populateSettingsForm() {
  ...
  const { drasticScore, runVerification, traitOptions, theme } = state.settings;
  ...
  if (form.themeAccent && theme) {
    form.themeAccent.value = theme.accent || DEFAULT_THEME.accent;
  }
  if (form.themeAccentStrong && theme) {
    form.themeAccentStrong.value = theme.accentStrong || theme.accent || DEFAULT_THEME.accentStrong;
  }
  if (form.themeBackground && theme) {
    form.themeBackground.value = theme.background || DEFAULT_THEME.background;
  }
  if (form.themePanel && theme) {
    form.themePanel.value = theme.panel || DEFAULT_THEME.panel;
  }
}
...
function renderRecruitStatus() {
  if (!elements.recruitStatus) return;
  if (!recruitProfileId) {
    elements.recruitStatus.innerHTML = '<div class="empty-state">Create a recruit profile to track your application.</div>';
    return;
  }
  if (!state.recruitStatus.length) {
    elements.recruitStatus.innerHTML = '<div class="empty-state">No applications yet—submit one to light up the mailbox.</div>';
    return;
  }
  elements.recruitStatus.innerHTML = state.recruitStatus
    .map((entry) => {
      const decisionTrail = (entry.reviewerNotes || [])
        .map((note) => `<li>${escapeHTML(note.note || '')} · ${escapeHTML(note.reviewerId || '')} · ${formatDate(note.createdAt)}</li>`)
        .join('');
      return `
        <article class="card search-result">
          <h4>${escapeHTML(entry.code)}</h4>
          <p class="subtitle">Status: ${escapeHTML(entry.status)}</p>
          <ul>${decisionTrail || '<li>Waiting for an officer review.</li>'}</ul>
        </article>
      `;
    })
    .join('');
...
function setSession(session) {
  const guildName = session.guildName || state.session.guildName || DEFAULT_GUILD_NAME;
  state.session = { ...state.session, ...session, guildName, authed: true };
  document.body.classList.remove('logged-out');
  document.body.classList.remove('recruit-portal');
  ...
  const persistable = {
    ...
    officerKey: getOfficerKey()
  };
  localStorage.setItem(SESSION_KEY, JSON.stringify(persistable));
  document.title = `${guildName} | Guild Nexus Honor Deck`;
}
...
function hydrateRecruitProfileFromStorage() {
  const stored = localStorage.getItem(RECRUIT_PROFILE_DATA_KEY);
  if (!stored) return;
  try {
    const profile = JSON.parse(stored);
    state.recruitProfile = profile;
    if (profile?.id) {
      recruitProfileId = profile.id;
      localStorage.setItem(RECRUIT_PROFILE_KEY, profile.id);
    }
    applyRecruitProfileToForms();
  } catch (error) {
    console.warn('Unable to parse recruit profile', error);
  }
}

function enterRecruitOnly() {
  document.body.classList.remove('logged-out');
  document.body.classList.add('recruit-portal');
  if (elements.loginScreen) {
    elements.loginScreen.setAttribute('aria-hidden', 'true');
  }
  setView('recruit');
  loadRecruitStatus();
  showToast('Fill out the application to join the guild.', 'info');
}

function logout() {
  state.session = { authed: false, username: '', guildName: '' };
  ...
  showToast('Signed out. Come back when ready.', 'info');
}
...
if (elements.loginForm) {
  elements.loginForm.addEventListener('submit', (event) => {
    ...
    const memberKey = elements.loginAccessKey?.value.trim();
    if (!username || !password || !memberKey) {
      showToast('Username, password, and guild key are required.', 'error');
      return;
    }
    setMemberKey(memberKey);
    setSession({ guildName: DEFAULT_GUILD_NAME, username });
    loadData();
  });
}

if (elements.openRecruitLink) {
  elements.openRecruitLink.addEventListener('click', () => {
    enterRecruitOnly();
  });
}
...
if (elements.recruitProfileForm) {
  elements.recruitProfileForm.addEventListener('submit', async (event) => {
    event.preventDefault();
    const payload = formToJSON(event.target);
    if (!payload.displayName || !payload.discordTag) {
      showToast('Display name and Discord tag are required.', 'error');
      return;
    }
    try {
      const recruit = await fetchJSON('/recruits/register', { method: 'POST', body: JSON.stringify(payload) });
      recruitProfileId = recruit.id;
      state.recruitProfile = recruit;
      localStorage.setItem(RECRUIT_PROFILE_KEY, recruit.id);
      localStorage.setItem(RECRUIT_PROFILE_DATA_KEY, JSON.stringify(recruit));
      applyRecruitProfileToForms();
      showToast('Recruit profile saved—now finish the application.', 'success');
      loadRecruitStatus();
    } catch (error) {
      showToast(error.message, 'error');
    }
...
['availabilityLocal', 'availabilityLocalEnd', 'timeZone'].forEach((name) => {
  if (elements.applicationForm?.elements[name]) {
    elements.applicationForm.elements[name].addEventListener('change', updateAvailabilityPreview);
  }
});
...
if (elements.applicationScheduleForm) {
  elements.applicationScheduleForm.addEventListener('submit', async (event) => {
    ...
    if (!recruitProfileId || !state.recruitProfile) {
      showToast('Save a recruit profile first so we can track your application.', 'error');
      return;
    }
    const basics = elements.applicationForm ? formToJSON(elements.applicationForm) : {};
    const schedule = formToJSON(event.target);
    const classSelections = collectCheckedValues(elements.applicationForm, 'classSelections');
    const roleSelections = collectCheckedValues(elements.applicationForm, 'roleSelections');

    if (!classSelections.length) {
      showToast('Pick at least one class so officers know your mains.', 'error');
      return;
    }
    if (!roleSelections.length) {
      showToast('Pick at least one role style (melee, caster, support, ranged).', 'error');
      return;
    }
    ...
    const payload = {
      ...basics,
      ...schedule,
      playerId: recruitProfileId,
      applicantName: state.recruitProfile.displayName,
      discordTag: state.recruitProfile.discordTag,
...
document.getElementById('settingsForm').addEventListener('submit', async (event) => {
  ...
  const payload = {
    ...
    theme: {
      accent: formData.get('themeAccent') || DEFAULT_THEME.accent,
      accentStrong: formData.get('themeAccentStrong') || formData.get('themeAccent') || DEFAULT_THEME.accentStrong,
      background: formData.get('themeBackground') || DEFAULT_THEME.background,
      panel: formData.get('themePanel') || DEFAULT_THEME.panel
    },
    traitOptions,
    adminPlayerId: getActorId()
  };
...
persistFormState('loginForm', 'draftLogin');
persistFormState('applicationForm', 'draftApplicationBasics');
persistFormState('applicationSchedule', 'draftApplicationSchedule');
persistFormState('runForm', 'draftRun');
persistFormState('reportForm', 'draftReport');
persistFormState('lfgPostForm', 'draftLfg');
...
async function loadRecruitStatus() {
  ...
  try {
    const statuses = await fetchJSON(`/recruits/status?playerId=${recruitProfileId}`);
    state.recruitStatus = statuses;
    renderRecruitStatus();
  } catch (error) {
    console.warn('Unable to load recruit status', error);
  }
}

function applyRecruitProfileToForms() {
  if (!state.recruitProfile) return;
  const { displayName, discordTag, note } = state.recruitProfile;
  if (elements.recruitProfileForm) {
    if (elements.recruitProfileForm.elements.displayName) {
      elements.recruitProfileForm.elements.displayName.value = displayName || '';
    }
    if (elements.recruitProfileForm.elements.discordTag) {
      elements.recruitProfileForm.elements.discordTag.value = discordTag || '';
    }
    if (elements.recruitProfileForm.elements.note) {
      elements.recruitProfileForm.elements.note.value = note || '';
    }
  }
}
...
function populateTimeZoneSelect() {
  ...
  const zones =
    (typeof Intl !== 'undefined' && Intl.supportedValuesOf ? Intl.supportedValuesOf('timeZone') : fallbacks) || fallbacks;
...
function updateAvailabilityPreview() {
  ...
  const start = elements.applicationForm.elements.availabilityLocal?.value;
  const end = elements.applicationForm.elements.availabilityLocalEnd?.value;
  const tz = elements.applicationForm.elements.timeZone?.value;
  const previewField = elements.applicationForm.elements.availabilityPreview;
  if (!previewField) return;
  const localWindow = formatLocalWindow(start, end);
  const eastern = convertWindowToEastern(localWindow, tz);
  previewField.value = eastern || 'Local window shown in ET when ready';
}
...
function bootstrap() {
  ...
  if (savedSession) {
    if (savedSession.username && elements.loginUsername) {
      elements.loginUsername.value = savedSession.username;
    }
    if (savedSession.memberKey && elements.loginAccessKey) {
      elements.loginAccessKey.value = savedSession.memberKey;
      setMemberKey(savedSession.memberKey);
    }
  }
  hydrateRecruitProfileFromStorage();
