...
const elements = {
  ...
  loginOfficerKey: document.getElementById('loginOfficerKey'),
...
function renderMetrics() {
  ...
  const metrics = [
    ...
    { label: 'Honor bank', value: totalHonor }
  ];
...
if (elements.loginForm) {
  elements.loginForm.addEventListener('submit', (event) => {
    ...
    if (!username || !password || !memberKey) {
      showToast('Username, password, and access key are required.', 'error');
      return;
    }
    setMemberKey(memberKey);
    const officerKey = elements.loginOfficerKey?.value || '';
    if (officerKey) {
      setOfficerKey(officerKey);
    }
...
function bootstrap() {
  ...
  if (savedSession) {
    ...
    if (savedSession.officerKey && elements.loginOfficerKey) {
      elements.loginOfficerKey.value = savedSession.officerKey;
      setOfficerKey(savedSession.officerKey);
    }
  }
