...
const elements = {
  ...
  topbarPresenceDetail: document.getElementById('topbarPresenceDetail'),
  topbarPresenceToggle: document.getElementById('presenceToggle'),
  topbarPresencePanel: document.getElementById('presencePopover'),
  topbarPresenceList: document.getElementById('presencePopoverList'),
...
function renderPresence() {
  if (!elements.presenceList || !elements.presenceSummary) return;
  ...
  if (elements.topbarOnline) {
    elements.topbarOnline.textContent = `${onlinePlayers.length} online · ${inGamePlayers.length} in game`;
  }
  if (elements.topbarPresenceDetail) {
    const short = [];
    if (inGamePlayers.length) short.push(`${inGamePlayers.length} in game`);
    if (onlinePlayers.length - inGamePlayers.length > 0) {
      short.push(`${onlinePlayers.length - inGamePlayers.length} online`);
    }
    elements.topbarPresenceDetail.textContent = short.length ? short.join(' · ') : 'No one online';
  }
  ...
  if (!onlinePlayers.length) {
    ...
    if (elements.topbarPresenceList) {
      elements.topbarPresenceList.innerHTML = '<li class="presence-empty">The hall is quiet—no members online.</li>';
    }
    return;
  }
  ...
  if (elements.topbarPresenceList) {
    elements.topbarPresenceList.innerHTML = onlinePlayers
      .sort((a, b) => a.displayName.localeCompare(b.displayName))
      .map((player) => {
        const presence = player.presence;
        const label = presence.state === 'in_game' ? 'In game' : 'Online';
        const source = presence.source ? ` · ${presence.source}` : '';
        const note = presence.note ? `<p class="subtitle">${escapeHTML(presence.note)}</p>` : '';
        return `
          <li class="presence-row ${presence.state}">
            <div>
              <strong>${escapeHTML(player.displayName)}</strong>
              <p class="subtitle">${escapeHTML(label)}${escapeHTML(source)}</p>
              ${note}
            </div>
            <span class="dot ${presence.state || 'online'}" aria-hidden="true"></span>
          </li>
        `;
      })
      .join('');
...
function renderInbox() {
  ...
  elements.inboxBadge.classList.toggle('muted', items.length === 0);
  ...
  elements.applicationList.innerHTML = filtered
    ...
    .map((application) => {
      ...
      const trail = (application.reviewTrail || [])
        .map((entry) => `<li>${escapeHTML(entry.status)} · ${formatDate(entry.createdAt)} — ${escapeHTML(entry.note)}</li>`)
        .join('');
      return `
        <article class="card application-card">
          <div>
            ...
            ${trail ? `<ul class="subtitle">${trail}</ul>` : ''}
          </div>
          <div>
...
async function loadData(showToastMessages = true) {
  try {
...
if (elements.topbarPresenceToggle) {
  elements.topbarPresenceToggle.addEventListener('click', () => {
    const isHidden = elements.topbarPresencePanel.hidden;
    elements.topbarPresencePanel.hidden = !isHidden;
    elements.topbarPresenceToggle.setAttribute('aria-expanded', String(isHidden));
  });
}
...
document.addEventListener('click', (event) => {
  ...
  if (
    elements.inboxPanel &&
    !elements.inboxPanel.hidden &&
    !elements.inboxPanel.contains(target) &&
    elements.inboxToggle &&
    !elements.inboxToggle.contains(target)
  ) {
    elements.inboxPanel.hidden = true;
    elements.inboxToggle.setAttribute('aria-expanded', 'false');
  }
  if (
    elements.topbarPresencePanel &&
    !elements.topbarPresencePanel.hidden &&
    !elements.topbarPresencePanel.contains(target) &&
    elements.topbarPresenceToggle &&
    !elements.topbarPresenceToggle.contains(target)
  ) {
    elements.topbarPresencePanel.hidden = true;
    elements.topbarPresenceToggle.setAttribute('aria-expanded', 'false');
  }
});
  ...
  elements.applicationList.addEventListener('submit', async (event) => {
    ...
    try {
      await fetchJSON(`/applications/${form.dataset.id}`, {
        method: 'PUT',
        body: JSON.stringify({ status: data.status, note: data.note, adminPlayerId })
      });
...
document.getElementById('characterForm').addEventListener('submit', async (event) => {
  ...
  if (!data.clazz) {
    showToast('Pick a class for this character', 'error');
    return;
  }
  try {
